...
// admin-only check (based on your DB)
...
// DELETE movie by released_year (PK)
if (isset($_GET["delete"])) {
  $releasedYear = $_GET["delete"]; // yyyy-mm-dd

  $pdo->beginTransaction();
  try {
    // must delete children first because of FK constraints
    ...
    // log action (optional). Since we deleted add_remove rows, we can log again:
    // But FK needs the movie_catalogue row, so logging AFTER delete would fail.
    // So we only log adds. (Delete logging would require a different table.)
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
  ...
  // simple validation: at least one movie name
  if ($released_year === "" || ($on_movie === "" && $up_movie === "")) {
    ...
    try {
      ...
      // Log add action in add_remove (your DB design)
